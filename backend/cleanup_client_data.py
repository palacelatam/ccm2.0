    ...
    def cleanup_client_data(self, client_id: str, confirm: bool = False) -> dict:
        ...
        try:
            ...
            for trade_doc in matched_trades:
                ...
                logger.info(f"{'Reset' if confirm else 'Would reset'} trade: {trade_doc.id} ({trade_doc.get('TradeNumber', 'Unknown')})")
            ...
            if not confirm:
                logger.info("\nThis was a DRY RUN. Use --confirm to actually perform the cleanup.")
            else:
...
def main():
    ...
    # Show current stats
    print("Current client statistics:")
